...
public class FeedNoticias {
    // lista de autores e mensagens publicadas no Feed
    ...
    /**
     * Sobrecarga que recebe um ator por parâmetro e retorna apenas as mensagens dele
     * @return A lista de publicações do Feed
     */
    public List<Publicacao> getPublicacoes(String autor) {
        // Cria uma lista de publicações somente-leitura
        List<Publicacao> publicacoes = new ArrayList<>();
        // Adiciona as mensagens do autor na lista 
     ...
     * @param autor nome do autor da publicação
     */
    private void addAutor(String autor){
        boolean encontrou = false;
        for (String autorA : autores){
            if (autorA.equals(autor)){
                encontrou = true;
            }
        }
        if (!encontrou){
